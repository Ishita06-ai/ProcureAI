...
export const PO_STATUS = {
  ...
  Rejected:    { cls: 'bg-rose-500/15 text-rose-400 border-rose-500/30' },
...
// PO approval workflow — role labels + a mirror of the server's canApproveStep
// (server/utils/approval.js) so the UI offers Approve/Reject only to the
// current approver. Admin may act on any pending step.
...
export const canApprovePoStep = (userRole, step) =>
  !!step && step.status === 'pending' && (userRole === 'admin' || userRole === step.requiredRole);
...
// Default currency is INR with Indian digit grouping (₹1,25,000) to match the
// procurement workflow thresholds. Pass a currency to override (e.g. USD).
...
// Compact duration for SLA display: "53h" under 2 days, "3d" after.
